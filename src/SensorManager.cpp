 ...
 * @brief Versão otimizada: sem redundâncias, I2C centralizado, logs compactos, filtro O(1)
 */
...
#ifdef USE_SHT20
    _sht20Online = _initSHT20(); if (_sht20Online) sensorsFound++;
#endif
#ifdef USE_CCS811
    _ccs811Online = _initCCS811(); if (_ccs811Online) sensorsFound++;
#endif
...
void SensorManager::update() {
    ...
    if (currentTime - _lastReadTime >= SENSOR_READ_INTERVAL) {
        ...
        if (_mpu9250Online && !_mpu6050Online) {
            ...
            _accelX = _applyFilter(gValues.x, _accelXBuffer);
            _accelY = _applyFilter(gValues.y, _accelYBuffer);
            _accelZ = _applyFilter(gValues.z, _accelZBuffer);
            _gyroX = gyrValues.x; _gyroY = gyrValues.y; _gyroZ = gyrValues.z;
            _magX = magValues.x; _magY = magValues.y; _magZ = magValues.z;
        }
#endif
        if (_bmp280Online) {
            float temp = _bmp280.readTemperature(); float press = _bmp280.readPressure();
    ...
    if (_sht20Online && (currentTime - _lastSHT20Read >= SHT20_READ_INTERVAL)) {
        ...
        float temp = _sht20.getTemperature(); float hum = _sht20.getHumidity();
    ...
    if (_ccs811Online && (currentTime - _lastCCS811Read >= CCS811_READ_INTERVAL)) {
        ...
        if (_ccs811.available()) {
            if (!_ccs811.readData()) {
                float co2 = _ccs811.geteCO2(); float tvoc = _ccs811.getTVOC();
                if (!(isnan(co2) || isnan(tvoc))) {
                    _co2Level = co2; _tvoc = tvoc;
                }
            }
        }
    }
...
void SensorManager::resetMPU6050() { if (_mpu6050Online) _mpu6050Online = _initMPU6050(); }
void SensorManager::resetBMP280() { if (_bmp280Online) _bmp280Online = _initBMP280(); }
void SensorManager::resetAll() {
    _mpu6050Online = _initMPU6050(); _bmp280Online = _initBMP280();
#ifdef USE_MPU9250
...
// INICIALIZAÇÃO SENSORES -- versões enxutas
bool SensorManager::_initMPU6050() {
    ...
    if (_mpu6050.begin() == 0) {
        _mpu6050.calcOffsets(true, true); delay(100); _mpu6050.update(); return true;
    }
    return false;
}
bool SensorManager::_initBMP280() {
    ...
    for (uint8_t i = 0; i < 2; i++) {
        if (_bmp280.begin(addresses[i])) {
            _bmp280.setSampling(Adafruit_BMP280::MODE_NORMAL, Adafruit_BMP280::SAMPLING_X16, Adafruit_BMP280::SAMPLING_X16, Adafruit_BMP280::FILTER_X16, Adafruit_BMP280::STANDBY_MS_500); delay(100);
...
#ifdef USE_MPU9250
bool SensorManager::_initMPU9250() { if (_mpu9250.init()) { _mpu9250.setAccRange(MPU9250_ACC_RANGE_8G); _mpu9250.setGyrRange(MPU9250_GYRO_RANGE_500); _mpu9250.enableGyrDLPF(); _mpu9250.setGyrDLPF(MPU9250_DLPF_6); delay(100);
    xyzFloat gValues = _mpu9250.getGValues(); if (!isnan(gValues.x)) return true; } return false; }
#endif
#ifdef USE_SHT20
bool SensorManager::_initSHT20() { Wire.beginTransmission(SHT20_ADDRESS); if (Wire.endTransmission() == 0) { _sht20.begin(); delay(500); float testTemp = _sht20.getTemperature(); float testHum = _sht20.getHumidity(); if (!(isnan(testTemp)||isnan(testHum))) return true; } return false; }
#endif
#ifdef USE_CCS811
bool SensorManager::_initCCS811() { uint8_t addresses[] = {CCS811_ADDR_1, CCS811_ADDR_2}; for (uint8_t i = 0; i < 2; i++) { if (_ccs811.begin(addresses[i])) { uint32_t startTime = millis(); while (!_ccs811.available() && (millis() - startTime < 3000)) { delay(100); } if (_ccs811.available()) return true; } } return false; }
#endif
// AUXILIARES
...
#endif
    if (_consecutiveFailures >= 10) { resetAll(); _consecutiveFailures = 5; }
}
float SensorManager::_applyFilter(float newValue, float* buffer) {
    static float sumX = 0, sumY = 0, sumZ = 0;
    if (buffer == _accelXBuffer) { sumX -= buffer[_filterIndex]; buffer[_filterIndex] = newValue; sumX += newValue; _filterIndex = (_filterIndex + 1) % CUSTOM_FILTER_SIZE; return sumX / CUSTOM_FILTER_SIZE; }
    if (buffer == _accelYBuffer) { sumY -= buffer[_filterIndex]; buffer[_filterIndex] = newValue; sumY += newValue; _filterIndex = (_filterIndex + 1) % CUSTOM_FILTER_SIZE; return sumY / CUSTOM_FILTER_SIZE; }
    if (buffer == _accelZBuffer) { sumZ -= buffer[_filterIndex]; buffer[_filterIndex] = newValue; sumZ += newValue; _filterIndex = (_filterIndex + 1) % CUSTOM_FILTER_SIZE; return sumZ / CUSTOM_FILTER_SIZE; }
