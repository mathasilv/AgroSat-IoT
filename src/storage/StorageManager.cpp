...
StorageManager::StorageManager()
    : _available(false), _rtcManager(nullptr), _systemHealth(nullptr),
      _lastInitAttempt(0), _totalWrites(0) {}
...
void StorageManager::_attemptRecovery() {
  ...
  if (now - _lastInitAttempt < REINIT_INTERVAL)
    return;
  ...
  if (begin()) {
    ...
    Serial.println(
        "[StorageManager] Recuperação falhou. Tentarei novamente em breve.");
  }
}
...
bool StorageManager::saveTelemetry(const TelemetryData &data) {
  ...
  if (!file) {
    Serial.println("[StorageManager] Erro de escrita (Telemetry). Marcando "
                   "como indisponível.");
    _available = false;
    return false;
  }
...
bool StorageManager::saveMissionData(const MissionData &data) {
  ...
  if (!file) {
    Serial.println("[StorageManager] Erro de escrita (Mission). Marcando como "
                   "indisponível.");
    _available = false;
    return false;
  }
...
bool StorageManager::saveLog(const String &message) {
  ...
  String ts = (_rtcManager && _rtcManager->isInitialized())
                  ? _rtcManager->getDateTime()
                  : String(millis());

  // FIX: Buffer local para thread-safety
  char localBuffer[512];
  snprintf(localBuffer, sizeof(localBuffer), "[%s] %s", ts.c_str(),
           message.c_str());
...
bool StorageManager::_checkFileSize(const char *path) {
  ...
  if (size > SD_MAX_FILE_SIZE) {
    ...
    if (strcmp(path, SD_LOG_FILE) == 0)
      ...
      createMissionFile();
    else if (strcmp(path, SD_SYSTEM_LOG) == 0)
      createLogFile();
  }
  return true;
}
...
void StorageManager::_formatTelemetryToCSV(const TelemetryData &data,
  ...
  // Parte 2: Temperaturas e pressão
  // (TempFinal,TempBMP,TempSI,Pressure,Altitude)
  pos += snprintf(buffer + pos, len - pos, "%.2f,%.2f,%.2f,%.1f,%.1f,",
                  sf(data.temperature), sf(data.temperatureBMP),
                  sf(data.temperatureSI), sf(data.pressure), sf(data.altitude));

  if (pos >= (int)len)
    return;

  // Parte 3: GPS (Lat,Lng,GpsAlt,Sats,Fix)
  ...
  // Parte 4: IMU (GyroX,GyroY,GyroZ,AccelX,AccelY,AccelZ,MagX,MagY,MagZ)
  pos +=
      snprintf(buffer + pos, len - pos,
               "%.2f,%.2f,%.2f,%.3f,%.3f,%.3f,%.1f,%.1f,%.1f,", sf(data.gyroX),
               sf(data.gyroY), sf(data.gyroZ), sf(data.accelX), sf(data.accelY),
               sf(data.accelZ), sf(data.magX), sf(data.magY), sf(data.magZ));

  if (pos >= (int)len)
    return;

  // Parte 5: Ambiente e status (Humidity,CO2,TVOC,Status,Errors,Payload)
  pos += snprintf(buffer + pos, len - pos, "%.1f,%.0f,%.0f,%d,%d,-,",
  ...
  // Parte 6: Sistema (Uptime,ResetCnt,MinHeap,CpuTemp)
  snprintf(buffer + pos, len - pos, "%lu,%d,%lu,%.1f", data.uptime,
           data.resetCount, data.minFreeHeap, sf(data.cpuTemp));
}
...
void StorageManager::_formatMissionToCSV(const MissionData &data, char *buffer,
  ...
  snprintf(buffer, len,
           ...
           data.ambientTemp,       // AmbTemp
           data.humidity,          // Humidity
           data.irrigationStatus,  // Irrigation
           data.rssi,              // RSSI
           data.snr,               // SNR
           data.packetsReceived,   // PktsRx
           data.packetsLost,       // PktsLost
           data.lastLoraRx,        // LastRx
           data.nodeTimestamp,     // NodeOriginTS
           data.collectionTime,    // SatArrivalTS
           data.retransmissionTime // SatTxTS
  );
}
