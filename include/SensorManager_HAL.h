/**
 * @file SensorManager_HAL.h
 * @brief SensorManager com HAL - Hardware Abstraction Layer
 * @details Versão refatorada usando dependency injection
 * 
 * MIGRAÇÃO GRADUAL:
 * Este arquivo NÃO substitui o original ainda.
 * Use-o como referência para migração incremental.
 * 
 * @author AgroSat-IoT Team
 * @date 2025-11-27
 */

#ifndef SENSORMANAGER_HAL_H
#define SENSORMANAGER_HAL_H

#include <Arduino.h>
#include "hal/interface/I2C.h"  // +++ HAL Interface
...
public:
    /**
     * @brief Construtor com Dependency Injection
     * @param i2c_hal Ponteiro para interface HAL I2C (abstrata)
     * 
     * EXEMPLO DE USO:
    ...
    bool calibrateIMU();
    void scanI2C();  // Agora usa HAL!
    void forceReinitBMP280();

private:
    // +++ HAL INTERFACE (dependency injection)
    HAL::I2C* _i2c_hal;
    
    // OBJETOS DE HARDWARE (mantidos - usam TwoWire interno)
    MPU9250_WE _mpu9250;
    ...
    bool _initMPU9250();
    bool _initBMP280();
    bool _initSI7021();  // +++ Agora usa HAL
    bool _initCCS811();
    
    // MÉTODOS INTERNOS - ATUALIZAÇÃO
    void _updateIMU();
    void _updateBMP280();
    void _updateSI7021();  // +++ Agora usa HAL
    void _updateCCS811();
